...
##获取代理设置
url = 'http://127.0.0.1:8080/get_all'

def get_proxy():
    response = requests.get(url)
    proxies = response.json()['proxies']
    http = [i for i in proxies if i.startswith('http://')]
    https = [i for i in proxies if i.startswith('https://')]
    proxies = {'http':random.choice(proxies),
               'https':random.choice(proxies),
        }
    requests.get('https://httpbin.org/get', proxies=proxies, timeout=5).json()['origin']
...
def word_count(text):
    ...
    words = Counter(words)
    for i, I in zip(areas, [i.capitalize() for i in areas]):
...
if __name__ == '__main__':
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        #保存位置
        store.to_excel(r'D:\store.xlsx', index=False)
        #无报表的cik集合
