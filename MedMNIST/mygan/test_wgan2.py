        ...
        for epoch in range(total_epochs):
            ...
            for i, (imgs, _) in enumerate(train_loader):
                ...
                real_grad = autograd.grad(
                    real_validity.unsqueeze(1), real_imgs, real_grad_out, create_graph=True,  retain_graph=True, only_inputs=True
                )[0]
                real_grad_norm = real_grad.view(real_grad.size(0), -1).pow(2).sum(1) ** (p / 2)
        
                fake_grad_out = Variable(Tensor(fake_imgs.size(0), 1).fill_(1.0), requires_grad=False)
                fake_grad = autograd.grad(
                    fake_validity.unsqueeze(1), fake_imgs, fake_grad_out, create_graph=True, retain_graph=True, only_inputs=True
                )[0]
