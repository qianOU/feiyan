...
def word_count(text):
    ...
    words = [i.lower() for i in re.findall(r'\w+', text)]   #将所有字符转换成小写字符
    words = Counter(words)
...
url = 'https://www.sec.gov/Archives/edgar/data/0000001750/000110465920085310/air-20200531x10k.htm'
...
if __name__ == '__main__':
    ...
    data3 = data2.drop_duplicates('CIK') #按 CIK列去重一下
    print(data3.CIK.values.dtype) #查看CIK列的数据类型
    data3['CIK'] = data3.CIK.apply(int) #将CIK列数据类型转换成int类型，防止exception， 只接受类型是int
    print(data3.CIK.values.dtype)
    threshold = data3.shape[0]
    wanted = data3.CIK.values.tolist() #取出data的CIK列
    round_ = 0
    no_tablbes = [] #没有报表的集合
    with ProcessPoolExecutor(max_workers=5) as executor:
        while len(wanted):
            ...
            print(len(wanted)) #输出要查询的CIK的长度
            futures = [executor.submit(map_main, cik) for cik in wanted]
